...
type View = 'login' | 'register' | 'checking-profile' | 'profile-error' | 'meal-plan' | 'profile' | 'progress' | 'admin'

function App() {
  const [view, setView] = useState<View>(() => getToken() ? getRole() === 'ADMIN' ? 'admin' : 'checking-profile' : 'login')
  ...
  return <LoginPage initialEmail={loginEmail} notice={loginNotice} onAuthenticated={(role) => { setLoginNotice(undefined); if (role === 'ADMIN') setView('admin'); else void checkProfile() }} onRegister={() => { setLoginNotice(undefined); setView('register') }}/>
}
export default App
