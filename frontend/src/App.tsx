...
type View = 'login' | 'register' | 'checking-profile' | 'profile-error' | 'profile' | 'progress'
...
function App() {
  ...
  const loadProfile = useCallback(async () => {
    try { const response = await getProfile(); setProfile(response.profile); setView('progress') }
    catch (error) {
  ...
  if (view === 'profile' && getToken()) return <ProfilePage profile={profile} required={!profile} onLogout={logout} onProgress={() => setView('progress')} onSaved={(savedProfile, wasFirstProfile) => { setProfile(savedProfile); if (wasFirstProfile) { setProgressNotice('Perfil criado com sucesso.'); setView('progress') } }}/>
  if (view === 'progress' && getToken() && profile) return <ProgressPage notice={progressNotice} onProfile={() => { setProgressNotice(undefined); setView('profile') }} onLogout={logout}/>
