...
interface ProgressPageProps { notice?: string; onMealPlan(): void; onProfile(): void; onLogout(): void }
export function ProgressPage({ notice, onMealPlan, onProfile, onLogout }: ProgressPageProps) {
  ...
  return <div className="app-shell"><header className="topbar"><div className="topbar__content"><div className="brand"><span className="brand__mark" aria-hidden="true">N</span><span>Nutri-AI</span></div><nav className="topbar__actions" aria-label="Navegação principal"><button className="nav-button" type="button" onClick={onMealPlan}>Plano alimentar</button><button className="nav-button" type="button" onClick={onProfile}>Perfil</button><button className="nav-button nav-button--active" type="button">Evolução</button><button className="logout-button" type="button" onClick={onLogout}>Sair</button></nav></div></header>
    <main className="page"><div className="page-heading"><div><p className="eyebrow">Histórico / Evolução</p><h1>Seu caminho, em números.</h1><p>Acompanhe seu peso ao longo do tempo e reconheça cada passo da sua evolução.</p></div></div>
      {error && <div className="notice notice--error" role="alert">{error}</div>}{(success || notice) && <div className="notice notice--success" role="status">{success || notice}</div>}
